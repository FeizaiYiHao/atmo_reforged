...
pub open spec fn syscall_new_proc_with_endpoint_requirement(old:Kernel, target_thread_ptr: ThreadPtr, endpoint_index: EndpointIdx, pt_regs:Registers, va_range:VaRange4K) -> bool {
    ...
    if old.get_is_process_thread_list_full(src_proc_ptr){
        false
    }else if old.get_container_quota(src_container_ptr) < va_range.len * 3 + 3{
        false
    }else if old.get_is_scheduler_full(src_container_ptr) {
        false
    }else if old.get_is_proc_list_full(src_container_ptr) {
        false
    }else if old.get_num_of_free_pages() < va_range.len * 3 + 3{
        false
...
pub open spec fn syscall_new_proc_with_endpoint_spec(old:Kernel, new:Kernel, target_thread_ptr: ThreadPtr, endpoint_index: EndpointIdx, pt_regs:Registers, va_range:VaRange4K, ret: SyscallReturnStruct) -> bool {
    let src_proc_ptr = old.get_thread(target_thread_ptr).owning_proc;
    let src_pcid = old.get_proc(src_proc_ptr).pcid;
    let src_container_ptr = old.get_thread(target_thread_ptr).owning_container;
    ...
    &&&
    syscall_new_proc_with_endpoint_requirement(old, target_thread_ptr, endpoint_index, pt_regs, va_range) == false ==> old == new
    &&&
    syscall_new_proc_with_endpoint_requirement(old, target_thread_ptr, endpoint_index, pt_regs, va_range) ==>
        ...
        &&
        new.get_proc(new_proc_ptr).owning_container == src_container_ptr
        &&
        new.get_container(src_container_ptr).owned_threads@ =~= old.get_container(src_container_ptr).owned_threads@.insert(new_thread_ptr)
        &&
        new.get_container(src_container_ptr).owned_procs@ =~= old.get_container(src_container_ptr).owned_procs@.push(new_proc_ptr)
        &&
        new.get_thread(new_thread_ptr).owning_container == src_container_ptr
        &&
...
pub fn syscall_new_proc_with_endpoint(&mut self, target_thread_ptr: ThreadPtr, endpoint_index: EndpointIdx, pt_regs:Registers, va_range:VaRange4K) ->  (ret: SyscallReturnStruct)
    ...
    ensures
        syscall_new_proc_with_endpoint_requirement(*old(self), target_thread_ptr, endpoint_index, pt_regs, va_range) == false <==> ret.is_error(),
        syscall_new_proc_with_endpoint_spec(*old(self), *self, target_thread_ptr, endpoint_index, pt_regs, va_range, ret),
{
    ...
    self.range_create_and_share_mapping(src_proc_ptr, &va_range, page_ptr_2, &va_range);

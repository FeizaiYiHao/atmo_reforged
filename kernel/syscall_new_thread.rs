...
pub open spec fn syscall_new_thread_spec(old:Kernel, new:Kernel, cpu_id: CpuId, pt_regs:Registers) -> bool{
    &&&
    old.wf()
    &&&
    new.wf()
...
impl Kernel{
    ...
    pub fn syscall_new_thread(&mut self, cpu_id: CpuId, pt_regs:Registers) -> (ret: SyscallReturnStruct)
        requires
            old(self).wf(),
            0 <= cpu_id < NUM_CPUS,
        ensures
    ...
    {
        let proc_ptr_op = self.proc_man.get_proc_ptr_by_cpu_id(cpu_id);
        if proc_ptr_op.is_none(){
            assert(self.wf());
            return SyscallReturnStruct::NoSwitchNew(ErrorCodeType::CpuIdle);
        }
        let proc_ptr = proc_ptr_op.unwrap();
        ...
        if self.proc_man.get_container_by_proc_ptr(proc_ptr).scheduler.len() >= CONTAINER_SCHEDULER_LEN {
            return SyscallReturnStruct::NoSwitchNew(ErrorCodeType::CpuIdle);
        } 
